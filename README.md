...
This code was developed with the help of GenAI (ChatGPT). GenAI was only used to assist with designing functions in Python. View [UBC's Principles on Generative AI](https://genai.ubc.ca/guidance/principles/).
...
### Setting Parameters
There are three parameters at the top of the code: BRIGHTNESS_PEAK_PROMINENCE, MIN_RADIUS, and FRAME_SKIP. Adjust these parameters based on the camera setup, lighting, and clarity of the freezing events.
...
Copy the relevant TSV and video file of each sample into a named subfolder. Organise all the data subfolders into one compiled folder with the code inside the compiled folder.
> Note: Each folder must contain only <ins>one</ins> TSV and video file. The presence of other files types does not affect the analysis.
...
The there are up to six input prompts for each sample. Fill them as necessary and press "Enter" (note: inputs cannot be changed after pressing enter). The prompts are:
...
> Only accepts whole numbers greater than 0
3. Manual droplet detection for {folder_name}? (Y/N)
> Entering "Y" or "y" will open the droplet selection window. Entering anything else will skip this step and continue the analysis.
...
> Enter the number of seconds in the video until the first freezing event. To ensure code doesn't miss the first droplet, enter a time that is ~5 before the first freezing event

The steps above will be prompted for every subfolder in the compiled data folder.

### Viewing the Results
When the code is finished running, it will download an Excel file in the compiled folder. Open this file to find the analysed data, sorted by freezing temperature and plate.
