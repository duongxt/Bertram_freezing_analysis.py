...
MIN_RADIUS = 8  # min radius for droplet detection (pixels)
# change this if droplets aren't being detected properly
FRAME_SKIP = 4 # number of frames to skip during analysis
# default value: 0 (analyse every frame)
...
for folder_name in os.listdir(base_dir): # cycle throug every subfolder in the same folder as the code
    ...
    binary_image = cv2.adaptiveThreshold(
        ...
        cv2.THRESH_BINARY, 79, -5
    )
    binary_vis = cv2.cvtColor(binary_image, cv2.COLOR_GRAY2BGR)
    
    # edge detection to run contours
    edges = cv2.Canny(blurred, 50, 150)
    
    # combine binary and edges
    merged_binary = cv2.bitwise_or(binary_image, edges)
    kernel = np.ones((3, 3), np.uint8)
    merged_binary = cv2.morphologyEx(merged_binary, cv2.MORPH_CLOSE, kernel, iterations=2)

    contours, _ = cv2.findContours(merged_binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    droplets = []
    for cnt in contours:
        area = cv2.contourArea(cnt)
        perimeter = cv2.arcLength(cnt, True) 
        if area < 20 or area > 1000 or perimeter < 40: # remove noise
            continue
    ...
    if FRAME_SKIP > 0:
        while True:
            ...
            if frame_idx < skip_seconds * fps: # skip frames before skip_seconds
                frame_idx += 1
                continue
    ...
    for droplet_id, brightness in droplet_brightness.items():
        ...
        if len(peaks) < 1 or np.max(deriv[peaks]) < BRIGHTNESS_PEAK_PROMINENCE:
            peak_info.append((droplet_id, None, None))  # Droplet didn't freeze (or under threshold)          
        else:
        ...
        if len(peaks_2) == 0:
            # No second peak after the first
            peak_info2.append((droplet_id, None, None))
        else:
            second_peak = np.max(deriv[peaks_2])
            second_peak_idx = np.argmax(deriv[peaks_2])
        ...
        plt.text(peak_sec, max_peak, str(droplet_id) + "_water", fontsize=8, color="k")
        plt.text(second_peak_sec, second_peak, str(droplet_id) + "_acid", fontsize=8, color="k")
    ...
    plt.title(f"{sample_name}: First Derivative of Brightness Peaks")
    plt.grid(True)
    plt.tight_layout()
    ...
    result_df_first = pd.DataFrame(peak_info, columns=["Droplet #", "Freezing Time (s)", "Freezing Temp (C)"])
    result_df_second = pd.DataFrame(peak_info2, columns=["Droplet #", "Freezing Time (s)", "Freezing Temp (C)"])

    # sort by freezing temp
    result_df_first = result_df_first.sort_values(by="Freezing Temp (C)", ascending=False, na_position='first').reset_index(drop=True)
    result_df_first["Frozen #"] = result_df_first["Freezing Temp (C)"].notna().cumsum().astype("Int64")
    result_df_first["Plate"] = result_df_first["Droplet #"].map(droplet_to_plate)
    
    result_df_second = result_df_second.sort_values(by="Freezing Temp (C)", ascending=False, na_position='first').reset_index(drop=True)
    result_df_second["Frozen #"] = result_df_second["Freezing Temp (C)"].notna().cumsum().astype("Int64")
    result_df_second["Plate"] = result_df_second["Droplet #"].map(droplet_to_plate)

    all_results_by_folder[folder_name] = result_df_first.copy()
    all_results_by_folder2[folder_name] = result_df_second.copy()
    
    # sort by plate number
    result_sorted_by_plate_first = result_df_first.sort_values(by=["Plate", "Droplet #"]).reset_index(drop=True)
    all_results_by_folder[folder_name + "_by_plate"] = result_sorted_by_plate_first
    
    result_sorted_by_plate_second = result_df_second.sort_values(by=["Plate", "Droplet #"]).reset_index(drop=True)
    all_results_by_folder2[folder_name + "_by_plate"] = result_sorted_by_plate_second
...
if all_results_by_folder:
    ...
    excel_filename = f"{today_str}_water_freezing_results.xlsx"
    ...
    print("No water results found across all folders.")

if all_results_by_folder:
    today_str = datetime.today().strftime("%Y_%m_%d")
    excel_filename = f"{today_str}_nitric_acid_freezing_results.xlsx"
    writer = pd.ExcelWriter(excel_filename, engine='openpyxl')
    for folder, df_sheet in all_results_by_folder.items():
        sheet_name = folder[:31]
        df_sheet.index = range(1, len(df_sheet) + 1)
